...
export class EnviarSolicitudContactoUseCase {
    ...
    async execute(usuarioId: number, contactoId: number): Promise<any> {
        ...
        if (usuarioId === contactoId) {
            throw new BadRequestException('No puedes enviar una solicitud a ti mismo');
        }

        // Verificar que el contacto existe
        const contacto = await this.comunidadRepository.verificarContactoExiste(contactoId);
        if (!contacto) {
            throw new BadRequestException('El contacto no existe');
        }

        // Verificar si ya existe solicitud pendiente
        const solicitudExistente = await this.comunidadRepository.obtenerSolicitudExistente(
            usuarioId,
            contactoId,
        );
        ...
        // Crear la solicitud
        return this.comunidadRepository.crearSolicitudContacto(usuarioId, contactoId);
    }
}
