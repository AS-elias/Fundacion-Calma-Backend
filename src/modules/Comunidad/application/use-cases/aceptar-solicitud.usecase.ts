...
export class AceptarSolicitudUseCase {
    ...
    async execute(solicitudId: number, usuarioId: number): Promise<any> {
        ...
        const solicitud = await this.comunidadRepository.obtenerSolicitudPorId(solicitudId);
        ...
        if (solicitud.estado !== 'pendiente') {
            throw new BadRequestException(
                `No se puede aceptar una solicitud en estado: ${solicitud.estado}`,
            );
        }

        // Actualizar estado
        return this.comunidadRepository.actualizarSolicitudContacto(solicitudId, 'aceptado');
    }
}
